...
| **Agent-Ready** | `lib/agentready.js` | Scores how legible a web page is to AI agents / answer engines (JSON-LD, content density, robots.txt AI-crawler access across 16 known tokens **classified by role** — blocking an answer-engine *retrieval* bot (OAI-SearchBot/Claude-SearchBot/PerplexityBot) is penalized harder than a training bot, since it's what actually kills citations — llms.txt, semantics) into a transparent 0–100 rubric with prioritized fixes. Pure `analyze()`; fetch is separate. |
...
| **Notify** | `lib/notify.js` | Delivers new alerts off the box to a Slack incoming-webhook and/or a generic webhook (via global `fetch`, zero-dep). The monitor CLI ships only *new* regressions (no re-spam); a dead channel is reported, not fatal; webhook URLs are redacted in logs. Configure under `notify` in `fleetdeck.config.json` (see `examples/`). |
