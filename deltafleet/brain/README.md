...
npm test                                                                 # 9 tests, node:test, zero deps
```
...
- **89.7× fewer tokens** (median) than dumping the store — evidence is ~389
  tokens vs ~33k.
- **12/12 answers correct.**
- **~0.7 ms** median retrieval.

That's the 5× memory/Claude-usage target cleared ~18×.
