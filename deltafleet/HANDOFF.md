...
| `deltafleet/fleetdeck/` | **Fleet Deck** — a zero-dep, local-first operator suite for AI-agent operators (a potential product line). Nine tools as **views over one shared spine** (append-only event log). See §5b. `node fleetdeck/bin/fleetdeck.js seed && … serve` → deck at :7420. 137 tests. |
