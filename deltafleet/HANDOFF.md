...
Scalability pass (**v0.14–v0.17**, ADR #25–#28, 118 tests): `Ledger.state()` is now an **incrementally-maintained materialized view** (O(1) amortized reads vs the old full-replay-per-call; ~10.4s→0.1ms for 200 reads over a 150k-event log); `/api/state` is **memoized by a ledger `version` counter** so N console pollers + idle ticks collapse to one recompute per append; **confidence calibration folds incrementally** so it's off the run hot path (was an O(all-actions) scan inside the agent loop); the **metric-sample window is bounded** (`Ledger.SAMPLE_WINDOW`, count preserved). Full ledger snapshot/segmentation is **deliberately deferred** (ADR #28) — the real bottlenecks are gone and doing it now would be premature bloat; the design is recorded for multi-year scale. The materialized view was adversarially sub-agent-reviewed (no reachable correctness bug; run-revival divergence is unreachable given unique run ids).
