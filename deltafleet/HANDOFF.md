...
Site v9 detail: home + `#/corridors` catalog + **3 corridor deep-dive pages** (`#/services/inbox-hygiene`, `#/services/document-intake`, `#/services/review-response`) + 4 marketing service pages reframed to corridor framing + About reframed + **2 integration pillar posts** (`how-to-integrate-ai-into-business-workflows`, `ai-invoice-processing-guide`). All six corridors now have a linked home. Home `<title>` fixed to integration-studio.
...
4. **Finish the v9 content cluster:** home, `#/corridors`, 3 corridor deep-dive pages, service-page reframe, About, and 2 pillar posts are DONE. Remaining: **3 more integration pillar posts** to hit the plan's target of 5 (candidates: "AI for lead response / speed-to-lead", "AI for weekly reporting", "AI for review management") — same answer-first format, wire into POSTS + ANSQ + llms.txt, router adds schema. Optional polish: the `#/services` hub page and archived pages still carry older framing (low priority — not in main nav).
